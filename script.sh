...
function checkinstalled {
    ...
    if [ $sshpassstatus == "installed" ]; then
...
function main {
    ...
    if [ "$IPcountry" == "SG" ]; then
        echo 'Your current IP is not anonymous'
        ...
        # Server will now obtain saved file via ftp from remote server.
        echo 'Obtaining saved file from remote server..'
        sleep 3

        # Using wget syntax to acquire the saved text file in the remote server.
        wget ftp://$ubuntuIP/whois.txt --ftp-user=$ubuntu_user --ftp-password=$ubuntu_pass 

        echo 'Files have been successfully saved into your current directory.'
        ls
        datetime=$(date)
        ...
        echo '[#] Activity has been successfully logged.'
    fi
}
main
